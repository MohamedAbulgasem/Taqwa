...
class TimelineBuilderTest {
    ...
    fun sunriseIsHiddenUnlessRequested() {
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_000_100), false)
    ...
    fun sunriseAppearsWhenRequested() {
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_000_100), true)
    ...
    fun partitionsIntoPassedCurrentAndUpcoming() {
        ...
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_012_600), false)
    ...
    fun countdownCountsDownToTheNextPrayer() {
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_007_200 - 600), false)
        assertEquals(600, s.countdown.inWholeSeconds)
    }

    @Test
    fun afterIshaTheNextPrayerIsTomorrowsFajr() {
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_018_100), false)
    ...
    fun beforeFajrNothingHasPassedYet() {
        val s = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(999_000), false)
    ...
    fun ringProgressRunsFromZeroToOneAcrossTheInterval() {
        ...
        val midway = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_003_600), false)
        assertTrue(midway.ringProgress in 0.4f..0.6f, "was ${midway.ringProgress}")
        val almost = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(1_007_100), false)
    ...
    fun ringProgressAdvancesBetweenMidnightAndFajr() {
        ...
        val later = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(999_600), false)
    ...
    fun ringProgressIsAlwaysWithinBounds() {
        listOf(999_000L, 1_000_000L, 1_012_600L, 1_018_100L, 1_085_000L).forEach { t ->
            val p = TimelineBuilder.build(yesterday, today, tomorrow, Instant.fromEpochSeconds(t), false).ringProgress
