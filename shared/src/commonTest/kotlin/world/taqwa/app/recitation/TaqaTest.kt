...
internal fun buildTaqa(
    ...
    version: Int = 1,
    magic: String = "TAQA",
    /** Ayah number to its measured length, as the pipeline writes since 13 September. */
    measuredMs: Map<Int, Long> = emptyMap(),
): ByteArray {
    val entries = StringBuilder()
    var off = 0L
    ayahs.forEachIndexed { i, bytes ->
        if (i > 0) entries.append(",")
        val own = measuredMs[i + 1]?.let { ",\"ms\":$it" }.orEmpty()
...
class TaqaDurationsTest {
    ...
    @Test
    fun aMeasuredLengthInTheIndexBeatsTheEstimate() {
        val fs = FakeFileSystem()
        val path = "/quran/ar.ahmedajamy/9.taqa".toPath()
        fs.createDirectories(path.parent!!)
        // Both 1,600 bytes, 100 ms by the estimate; the second was measured at 2,455 ms.
        val ayahs = listOf(ByteArray(1_600) { 1 }, ByteArray(1_600) { 2 })
        fs.write(path) { write(buildTaqa(surah = 9, kbps = 128, ayahs = ayahs, measuredMs = mapOf(2 to 2_455L))) }

        val taqa = TaqaFile(path, fs)

        assertEquals(2_455L, taqa.index().ayah(2)?.ms)
        assertEquals(mapOf(1 to 100L, 2 to 2_455L), taqa.ayahDurationsMs())
    }

    /** MPEG-1 Layer III, 44.1 kHz, [kbps], stereo unless [mono], with a Xing tag where a decoder looks for it. */
    private fun xingFrame(kbps: Int, mono: Boolean = false, tag: String = "Xing", padding: Int = 0): ByteArray {
        val rateIndex = listOf(0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320).indexOf(kbps)
        val length = 144 * kbps * 1000 / 44100 + padding
        val frame = ByteArray(length)
        frame[0] = 0xFF.toByte()
        frame[1] = 0xFB.toByte()                                         // MPEG-1, Layer III, no CRC
        frame[2] = ((rateIndex shl 4) or (padding shl 1)).toByte()      // 44.1 kHz
        frame[3] = (if (mono) 0xC0 else 0x00).toByte()
        val at = 4 + if (mono) 17 else 32
        tag.encodeToByteArray().copyInto(frame, at)
        return frame
    }

    @Test
    fun theXingFrameAfterTheTagIsMetadataNotSound() {
        assertEquals(417L, xingFrameBytes(xingFrame(128)))
        assertEquals(208L, xingFrameBytes(xingFrame(64)))
        assertEquals(209L, xingFrameBytes(xingFrame(64, padding = 1)))
        assertEquals(208L, xingFrameBytes(xingFrame(64, mono = true, tag = "Info")))
        // An ordinary audio frame carries no tag at that offset, and is left alone.
        assertEquals(0L, xingFrameBytes(xingFrame(64, tag = "\u0000\u0000\u0000\u0000")))
        assertEquals(0L, xingFrameBytes(ByteArray(40)))
        assertEquals(0L, xingFrameBytes(ByteArray(12) { 0xFF.toByte() }))
    }

    @Test
    fun theEstimateLeavesOutBothTheTagAndTheXingFrame() {
        val fs = FakeFileSystem()
        val path = "/quran/ar.alafasy/1.taqa".toPath()
        fs.createDirectories(path.parent!!)
        // 64 kbps: 8 bytes a millisecond. 407 bytes of tag, a 208-byte Xing frame, then 800 bytes
        // of audio, which is 100 ms — the tag and the frame would have added 77 ms.
        val ayah = id3(397) + xingFrame(64) + ByteArray(800) { 1 }
        fs.write(path) { write(buildTaqa(kbps = 64, ayahs = listOf(ayah))) }

        assertEquals(mapOf(1 to 100L), TaqaFile(path, fs).ayahDurationsMs())
    }
