...
#   2. adds the app's InfoPlist.strings (en, ar) so the location permission sheet is in Arabic on
#      an Arabic phone;
...
# 2. the app's InfoPlist.strings, one variant group with a file per language
variant = app_group.children.find do |child|
  child.is_a?(Xcodeproj::Project::Object::PBXVariantGroup) && child.name == 'InfoPlist.strings'
end
variant ||= app_group.new_variant_group('InfoPlist.strings')
...
add_once(app.resources_build_phase, variant)
