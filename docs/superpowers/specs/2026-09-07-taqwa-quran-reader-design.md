...
- **Lines**: 15 per page (pages 1 and 2 have fewer and are vertically centred). Line types: `surah` (a band: hairline-framed row with the surah name in the Mushaf font centred, ayah count and Makki/Madani in 11 sp on the sides), `basmala` (centred), `text` (the words of that printed line). Text lines are set **justified** to the frame width; a line that ends a surah and the last line of Al-Fatiha's page are not justified but aligned to the start. Every line is a single `BasicText` with `maxLines = 1` and step-based auto-size so that no line ever wraps: the page's base size is derived from the frame width (28 sp at 375 dp, scaling linearly) and each line may shrink by up to 30% to fit.
...
One SQLite file, `shared/src/commonMain/composeResources/files/quran.db`, built by `tools/build-quran-db.py`, `PRAGMA user_version = 1`, vacuumed, journal off. Schema:
...
Quran text only ever uses the Hafs font. UI text stays Manrope / OS Arabic. Sizes: translation mode 22–40 sp (default 28), line height 2.0×; Mushaf page base size 28 sp at 375 dp width, scaled by width, with per-line auto-shrink to 70%; basmala 24 sp; surah names in lists 20 sp; header title in Arabic UI 20 sp.
