...
- A query containing any Arabic letter searches the **Arabic text** through the bundled FTS5 table (`ayah_fts` over `ayah.text_search`, Tanzil simple-clean). The query is folded with `QuranText.normaliseForSearch`, split on spaces, each token quoted and suffixed with `*` (prefix match), joined with spaces (implicit AND). So «الرحمن» finds every ayah with a word beginning الرحمن; «رب العالمين» finds ayahs containing both prefixes.
...
- **Quran.sq** gains `searchArabic: SELECT surah, number, text_uthmani FROM ayah WHERE rowid IN (SELECT rowid FROM ayah_fts WHERE ayah_fts MATCH ?) ORDER BY surah, number LIMIT ?;` and `translationTextsAll: SELECT surah, number, text FROM ayah_translation WHERE translation_id = ? ORDER BY surah, number;`. No schema change, no database rebuild, `user_version` stays 3.
- **QuranSource** gains `suspend fun searchArabic(query: String, limit: Int): List<SearchHit>` and `suspend fun searchTranslation(translationId: String, query: String, limit: Int): List<SearchHit>`, `data class SearchHit(val surah: Int, val ayah: Int, val arabic: String, val translation: String?)`. The FTS query string is built by a pure `SearchQuery.fts(raw: String): String?` (null when no token survives normalisation) so the quoting rule is testable without a database; `SearchQuery.isArabic(raw)` decides the branch.
