...
- **Store version 1.0.0 (25).** `scripts/bump-version.sh 1.0.0 25` once, when the store builds
  are cut; one commit feeds both stores. Codes 18–24 are spent on device builds (0.19.1 is code
  24; the S23 carries a 1.0.0-labelled code 20), so nothing below 25 goes anywhere.
...
- The store build: `scripts/test.sh`, `scripts/ios-build.sh`, `scripts/bump-version.sh 1.0.0 25`,
