...
- **Store version 1.0.0 (26).** `scripts/bump-version.sh 1.0.0 26` once, when the store builds
  are cut; one commit feeds both stores. Codes 18–25 are spent on device builds (0.20.0 is code
  25; the S23 carries a 1.0.0-labelled code 20), so nothing below 26 goes anywhere.
...
- The store build: `scripts/test.sh`, `scripts/ios-build.sh`, `scripts/bump-version.sh 1.0.0 26`,
