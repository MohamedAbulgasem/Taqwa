...
- **Store version 1.0.0 (24).** `scripts/bump-version.sh 1.0.0 24` once, when the store builds
  are cut; one commit feeds both stores. Codes 18–23 are spent on device builds (0.19.0 is code
  23; the S23 carries a 1.0.0-labelled code 20), so nothing below 24 goes anywhere.
...
- The store build: `scripts/test.sh`, `scripts/ios-build.sh`, `scripts/bump-version.sh 1.0.0 24`,
