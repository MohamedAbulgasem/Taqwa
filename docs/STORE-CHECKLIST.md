...
- **Store version 1.0.0 (23).** `scripts/bump-version.sh 1.0.0 23` once, when the store builds
  are cut; one commit feeds both stores. Codes 18–22 are spent on device builds (the S23 carries
  a 1.0.0-labelled code 20), so nothing below 21 would install there anyway.
...
- The store build: `scripts/test.sh`, `scripts/ios-build.sh`, `scripts/bump-version.sh 1.0.0 23`,
